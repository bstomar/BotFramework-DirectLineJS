...
import { BehaviorSubject, Observable, Subscription, of, empty, from, interval, Subscriber } from 'rxjs';
import { retryWhen, flatMap, filter, take, tap, map, mergeMap, delay, catchError, count, combineLatest, share } from 'rxjs/operators'
...
export class DirectLine implements IBotConnection {
  ...
  private checkConnection(once = false) {
    let obs = this.connectionStatus$.pipe(flatMap(connectionStatus => {
    ...
    })).pipe(filter(connectionStatus => connectionStatus != ConnectionStatus.Uninitialized && connectionStatus != ConnectionStatus.Connecting))

      .pipe(flatMap(connectionStatus => {
  ...
  private refreshTokenLoop() {
    this.tokenRefreshSubscription = interval(intervalRefreshToken).pipe(
    ...
    return this.checkConnection(true).pipe(
      flatMap(_ =>
        ...
        ajax({
          ...
          pipe(map(ajaxResponse => ajaxResponse.response.token as string)).
    ...
    return this.checkConnection(true).
      pipe(flatMap(_ =>
        ajax({
          ...
          pipe(map(ajaxResponse => ajaxResponse.response.id as string)).
          pipe(catchError(error => this.catchPostError(error)))
      )).
      pipe(catchError(error => this.catchExpiredToken(error)));
  }
    ...
    return this.checkConnection(true).
      pipe(flatMap(_ => {
        ...
        return from(attachments || []).
          pipe(flatMap((media: Media) =>
            ajax({
              method: "GET",
              url: media.contentUrl,
              responseType: 'arraybuffer'
            }).pipe(tap(ajaxResponse =>
  ...
  private pollingGetActivity$() {
    return interval(this.pollingInterval).
      pipe(combineLatest(this.checkConnection())).
      pipe(flatMap(_ =>
        ajax({
          method: "GET",
          ...
          pipe(map(ajaxResponse => ajaxResponse.response as ActivityGroup)).
          pipe(flatMap(activityGroup => this.observableFromActivityGroup(activityGroup)))
      )).
      pipe(catchError(error => empty()));
    //pipe(catchError(error => empty<Activity>()));
  }

  private observableFromActivityGroup(activityGroup: ActivityGroup) {
    if (activityGroup.watermark)
      this.watermark = activityGroup.watermark;
    return from(activityGroup.activities);
  }
    ...
    return this.checkConnection().
      pipe(flatMap(_ =>
        this.observableWebSocket<ActivityGroup>()
          ...
          .pipe(retryWhen(error$ => error$.pipe(mergeMap(error => this.reconnectToConversation()))))
      )).
      pipe(flatMap(activityGroup => this.observableFromActivityGroup(activityGroup)))
  }
  ...
  private observableWebSocket<T>() {
    return Observable.create((subscriber: Subscriber<T>) => {
      ...
      ws.onopen = open => {
        ...
        sub = interval(timeout).subscribe(_ => ws.send(""));
      }
